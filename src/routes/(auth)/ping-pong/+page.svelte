		...
		if (browser) {
			...
			function gameLoop() {
				if (window.innerWidth < 820) {
					...
					paddleHeight = 0.3 * canvas.height;
				}
				movePaddles();
				moveBall();
				drawCanvas();
...
>
	<div
		class="text-lg mx-auto text-center p-3 flex-col justify-center items-center hidden"
	>
		<svg
		...
		Ping pong game is under construction, come back later
	</div>
	<div
		class="w-full max-w-3xl flex justify-between space-x-10 px-10 items-center h-20 rounded-xl bg-[#ffffff79] blur-bg text-black absolute mx-auto top-28 z-50"
	>
	...
	<canvas
		id="pingPongCanvas"
		class="rounded-xl canvas-aspect-ratio relative mt-36"
	/>
	...
	{#if scoreDisplay && !gameEnded}
		<div
			...
			class="z-50 text-7xl text-center font-bold flex justify-center items-center absolute mx-auto my-auto"
		>
	...
	{#if !gameStarted}
		<div
			...
			class="z-50 text-center font-bold flex flex-col space-y-5 justify-center items-center absolute mx-auto my-auto"
		>
	...
	{#if gameEnded}
		<div
			in:fly={{ delay: 0, duration: 500, y: 300, opacity: 0 }}
			class="z-50 text-7xl text-center font-bold flex flex-col space-y-5 justify-center items-center absolute mx-auto my-auto"
		>
			{winner} Wins! <br />
			{playerScore} - {computerScore}
			<button
