...
<section class="w-full smokey-blue text-gray-200" style="height: 50rem;">
...
<section use:inview={options} on:inview_change={({detail})=>{sectionOneINVIEW = detail.inView}} class="w-full flex justify-center items-center bg-[#E5E5E5]" style="height: 30rem;" data-theme="custom"  >
  {#if sectionOneINVIEW}
<p in:fly={{duration:2000,y:500,opacity:.3,easing:cubicOut}} class="lg:text-3xl text-lg px-5 text-gray-900 font-bold text-center"> "On-scroll animations using svelte-inview and svelte transistions 😎" <br> - <span class="italic text-sm lg:text-xl">Elliot Morrison</span></p>
{/if}
</section>


<section use:inview={options} on:inview_change={({detail})=>{sectionTwoINVIEW = detail.inView}} class="w-full smokey-blue-inverse flex justify-center items-center" style="height: 60rem;" data-theme="custom">
  {#if sectionTwoINVIEW}
...
<section use:inview={options} on:inview_change={({detail})=>{sectionThreeINVIEW = detail.inView}} class="w-full flex justify-center items-center smokey-blue" style="height: 40rem;" data-theme="custom">
  {#if sectionThreeINVIEW}
  <p in:blur={{ amount: 10, duration: 1000}} class="lg:text-3xl text-lg px-5 text-gray-200 font-bold text-center"> "This is the end of the homepage 😁" <br> - <span class="italic text-sm lg:text-xl">Elliot Morrison</span></p>
  {/if}
  </section>
